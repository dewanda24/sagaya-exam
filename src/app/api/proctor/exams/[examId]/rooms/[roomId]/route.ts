...
export async function GET(
  ...
  try {
    ...
    const data = await ProctorCoreService.getPreExamCheck(auth.user.id, schoolId, examId, roomId);
    ...
    return NextResponse.json(
      { success: false, error: error.message || 'Gagal memuat pra-pemeriksaan ruang ujian.' },
      { status: 500 }
    );
  }
}
