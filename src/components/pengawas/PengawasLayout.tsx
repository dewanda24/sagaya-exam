...
import PengawasSidebar from './PengawasSidebar';
import PengawasHeader from './PengawasHeader';
...
export default function PengawasLayout({
  ...
  useEffect(() => {
    ...
    const saved = localStorage.getItem('sagaya_pengawas_sidebar_collapsed');
    if (saved === 'true') {
      setCollapsed(true);
    }
  ...
  const handleToggleCollapse = (val: boolean) => {
    setCollapsed(val);
    localStorage.setItem('sagaya_pengawas_sidebar_collapsed', String(val));
  };
